...
- **Tree diff integration tests** (`tests/tree_diff_test.rs`): fixture-driven tests of the topdiff engine with insta JSON and renderer snapshots
- **Tree diff property tests** (`tests/tree_diff_proptest.rs`): 13 tests (12 property-based + 1 fixed sanity check) verifying the banded/pruned algorithms against a naive Zhang-Shasha oracle, plus edit-mapping validity invariants
- **Benchmarks** (`benches/ast_navigation_bench.rs`, `benches/tree_diff_bench.rs`): criterion groups — parsing, cache hit/miss, symbols, node lookup, navigation, scope, queries, definitions, plus topdiff-vs-myers engine comparisons
- **Fuzz targets** (`fuzz/`): 4 libfuzzer targets — `fuzz_parse_and_navigate`, `fuzz_query`, `fuzz_node_to_info`, `fuzz_tree_diff`
...
**Test fixtures** live in `test_data/` — `ast_navigation/` (10 files: Rust, Python, TypeScript, Go, C++, C, Java, plus edge cases for deep nesting, Unicode, and empty files) and `tree_diff/` (before/after source pairs for the topdiff engine).
...
5. Output rendering (`render/`) — trait via `enum_dispatch`, implementations: `unified.rs`, `json.rs`, `structural.rs`
